<div class="admin-card rounded-[28px] p-5">
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-6">
        <div>
            ...
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">
                Publica actividades, fechas, fotografías y llamados a reserva.
            </p>
        </div>

        <a href="{{ route('eventos') }}" target="_blank"
    ...
    <div class="grid grid-cols-1 xl:grid-cols-[420px_1fr] gap-5">
        <div class="bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-slate-700 rounded-3xl p-5">
            <h3 class="text-sm font-black uppercase tracking-widest mb-4">Nuevo evento</h3>

            <form action="{{ route('admin.contenido.eventos.items.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                @csrf
                <input name="title" required placeholder="Título del evento"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <input name="category" placeholder="Etiqueta: Próximo, Taller..."
                ...
                <textarea name="body" rows="4" placeholder="Descripción del evento"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <input name="button_label" value="Agendar visita" placeholder="Texto botón"
                           class="bg-slate-50 dark:bg-[#07111f] border border-slate-200 dark:border-slate-700 rounded-2xl p-3 text-sm dark:text-white">
                    <input name="button_url" value="{{ route('reservas.create') }}" placeholder="URL botón"
                           class="bg-slate-50 dark:bg-[#07111f] border border-slate-200 dark:border-slate-700 rounded-2xl p-3 text-sm dark:text-white">
                </div>
        ...
        <div class="space-y-4">
            @forelse (($eventItems ?? collect()) as $item)
                    ...
                    <div>
                        <form action="{{ route('admin.contenido.items.update', $item) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                                <input name="category" value="{{ $item->category }}" placeholder="Etiqueta" class="bg-slate-50 dark:bg-[#07111f] border border-slate-200 dark:border-slate-700 rounded-2xl p-3 text-sm dark:text-white">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                ...
                                <input name="button_url" value="{{ $item->button_url }}" placeholder="URL botón" class="bg-slate-50 dark:bg-[#07111f] border border-slate-200 dark:border-slate-700 rounded-2xl p-3 text-sm dark:text-white">
                            </div>
                            <label class="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-300 font-bold">
                        ...
                        <form action="{{ route('admin.contenido.items.destroy', $item) }}" method="POST" class="mt-3">
                            @csrf
                            @method('DELETE')
                            <button onclick="return confirm('¿Eliminar este evento?')" class="bg-rose-600 hover:bg-rose-500 text-white rounded-2xl px-5 py-3 font-black uppercase tracking-widest text-xs">Eliminar</button>
                        </form>
                    </div>
                </div>
            @empty
